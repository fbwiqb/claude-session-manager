...
- 우측 패널: 메타 정보 + **본문 미리보기**(메시지 말풍선) + 버튼
  (⭐즐겨찾기 · ✏️리네임 · ▶️cmd에서 열기 · 🗑️휴지통).
- 본문 미리보기: 세션 선택 시 **가장 최신 대화가 보이도록 자동으로 맨 아래로 스크롤**,
  스크롤바로 위쪽 과거 대화를 더 볼 수 있음(고정 높이 미리보기 패널). 기본은 user/assistant
  대화만 표시, 토글로 tool 호출/결과까지 펼침.
